...
const HUM_PAGE_STORE_KEY = 'humidity_page_v1'
type Range = '1h' | '6h' | '12h' | '24h'

type LatestDoc = { h_air_pct?: number; ts?: { toDate?: () => Date } }
type TelemetryRow = { timestamp: Date; h_air: number | null }

export const HumidityPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      const raw = localStorage.getItem(HUM_PAGE_STORE_KEY)
      if (raw) {
      ...
      console.error('Error restaurando datos de humedad:', e)
    }
  ...
  useEffect(() => {
    ...
    const recompute = () => {
      ...
      merged.forEach((v) => {
        ...
        // Buscar humedad en varios lugares
        let hum = numOrNull(v.h_air_pct)
        if (hum == null && v.sensors && typeof v.sensors === 'object') hum = numOrNull(v.sensors.h_air_pct)
        if (hum == null && v.raw && v.raw.sensors && typeof v.raw.sensors === 'object') hum = numOrNull(v.raw.sensors.h_air_pct)
        if (hum == null && v.raw) hum = numOrNull(v.raw.h_air_pct)
  ...
  // Sin filtro de tiempo: graficar todos los datos
        rows.push({ timestamp: t, h_air: hum })
      })
  ...
  try {
    localStorage.setItem(HUM_PAGE_STORE_KEY, JSON.stringify({
      points: rows,
      selectedTimeRange
    }))
  } catch {}
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        const s = JSON.parse(raw)
        localStorage.setItem(HUM_PAGE_STORE_KEY, JSON.stringify({ points: s.points, selectedTimeRange }))
      }
    } catch (e) {
      console.error('Error guardando rango de humedad:', e)
    }
  }, [selectedTimeRange])
  ...
  /* ---------- Cálculos ---------- */
  const latestHumidity = useMemo(() => numOrNull(latest?.h_air_pct), [latest])
  ...
  const stats = useMemo(() => {
    const xs = filteredPoints.map(p => p.h_air).filter(isFiniteNum)
  ...
  const rangeWidth = useMemo(() => {
    if (!isFiniteNum(latestHumidity)) return 0
    const h = latestHumidity as number
    const min = 20, max = 100
    const optimalMin = 50, optimalMax = 65
    const currentPct = ((h - min) / (max - min)) * 100
    // Si está dentro del rango óptimo, mostrar el % dentro del mismo; si no, usar el de la humedad actual
    if (h >= optimalMin && h <= optimalMax) {
      return ((h - optimalMin) / (optimalMax - optimalMin)) * 100
    }
    ...
    <Layout isOnline={firestoreReady && points.length > 0 ? isOnline : undefined} statusHint={firestoreReady && points.length > 0 ? statusHint : undefined}>
      <div className="mb-8">
        <div className="flex items-center">
          <DropletIcon className="h-8 w-8 text-purple-500 mr-3" />
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Monitoreo de Humedad</h1>
        </div>
        <p className="text-gray-600 dark:text-gray-400 mt-2">Análisis detallado de humedad ambiente</p>
      </div>

      {/* Loading State */}
      {isLoading ? (
        <div className="space-y-4 sm:space-y-6">
          {/* Skeleton: tarjeta de humedad actual */}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 mb-4 sm:mb-6 transition-colors duration-200">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <div>
            <h2 className="text-base sm:text-lg font-semibold text-gray-700 dark:text-gray-200 mb-4">Humedad Actual</h2>
            <div className="flex items-center gap-4">
              <div className="p-3 sm:p-4 rounded-lg bg-purple-50 dark:bg-purple-900/20 flex-shrink-0">
                <DropletIcon className="h-8 sm:h-10 w-8 sm:w-10 text-purple-500" />
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-end gap-1">
                  <span className="text-3xl sm:text-4xl font-bold text-gray-800 dark:text-white truncate">
                    {isFiniteNum(latestHumidity) ? round1(latestHumidity as number) : 'N/D'}
                  </span>
                  <span className="text-lg sm:text-xl text-gray-500 dark:text-gray-400 flex-shrink-0">%</span>
                </div>
            ...
            <div className="mt-4">
              <div className="flex justify-between text-xs sm:text-sm mb-1">
                ...
                <span className="font-medium text-gray-700 dark:text-gray-300">50% - 65%</span>
              </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 mb-4 sm:mb-6 transition-colors duration-200">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-4">
          <h2 className="text-base sm:text-lg font-semibold text-gray-800 dark:text-white">Gráfico de Humedad</h2>
        ...
        <div className="h-64 sm:h-80 overflow-x-auto -mx-4 sm:-mx-6 px-4 sm:px-0">
          <ResponsiveContainer width="100%" height="100%" minWidth={300}>
            <LineChart data={chartData} margin={{ top: 10, right: 30, left: 20, bottom: 10 }}>
              ...
              />
              <YAxis tick={{ fontSize: 12, fill: '#64748b' }} axisLine={{ stroke: '#e5e7eb' }} tickLine={false} domain={[30, 70]} />
              <Tooltip
                labelFormatter={(ts) => {
