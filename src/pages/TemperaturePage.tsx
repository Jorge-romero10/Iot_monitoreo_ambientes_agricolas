...
const TEMP_PAGE_STORE_KEY = 'temp_page_v1'
type Range = '1h' | '6h' | '12h' | '24h'

type LatestDoc = { t_air_c?: number; ts?: { toDate?: () => Date } }
type TelemetryRow = { timestamp: Date; t_air: number | null }
...
export const TemperaturePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      const raw = localStorage.getItem(TEMP_PAGE_STORE_KEY)
      if (raw) {
      ...
      console.error('Error restaurando datos de temperatura:', e)
    }
  ...
  useEffect(() => {
    ...
    const recompute = () => {
      ...
      merged.forEach((v) => {
        ...
        // Buscar temperatura en varios lugares
        let temp = numOrNull(v.t_air_c)
        if (temp == null && v.sensors && typeof v.sensors === 'object') temp = numOrNull(v.sensors.t_air_c)
        if (temp == null && v.raw && v.raw.sensors && typeof v.raw.sensors === 'object') temp = numOrNull(v.raw.sensors.t_air_c)
        if (temp == null && v.raw) temp = numOrNull(v.raw.t_air_c)
  ...
  // Sin filtro de tiempo: graficar todos los datos
        rows.push({ timestamp: t, t_air: temp })
      })
  ...
  try {
    localStorage.setItem(TEMP_PAGE_STORE_KEY, JSON.stringify({
      points: rows,
      selectedTimeRange
    }))
  } catch {}
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        const s = JSON.parse(raw)
        localStorage.setItem(TEMP_PAGE_STORE_KEY, JSON.stringify({ points: s.points, selectedTimeRange }))
      }
    } catch (e) {
      console.error('Error guardando rango de temperatura:', e)
    }
  }, [selectedTimeRange])
  ...
  /* ---------- Cálculos ---------- */
  const latestTemp = useMemo(() => numOrNull(latest?.t_air_c), [latest])
  ...
  const stats = useMemo(() => {
    const xs = filteredPoints.map(p => p.t_air).filter(isFiniteNum)
  ...
  const rangeWidth = useMemo(() => {
    if (!isFiniteNum(latestTemp)) return 0
    const t = latestTemp as number
    const min = 15, max = 35
    const optimalMin = 20, optimalMax = 26
    const optimalPct = ((optimalMin - min) / (max - min)) * 100
    const optimalWidthPct = ((optimalMax - optimalMin) / (max - min)) * 100
    const currentPct = ((t - min) / (max - min)) * 100
    // Si está dentro del rango óptimo, mostrar el % dentro del mismo; si no, usar el de la temperatura actual
    if (t >= optimalMin && t <= optimalMax) {
      return ((t - optimalMin) / (optimalMax - optimalMin)) * 100
    }
    ...
    <Layout isOnline={firestoreReady && points.length > 0 ? isOnline : undefined} statusHint={firestoreReady && points.length > 0 ? statusHint : undefined}>
      <div className="mb-8">
        <div className="flex items-center">
          <ThermometerIcon className="h-8 w-8 text-blue-500 mr-3" />
          <h1 className="text-2xl font-bold text-gray-800">Monitoreo de Temperatura</h1>
        </div>
        <p className="text-gray-600 mt-2">Análisis detallado de temperatura ambiente</p>
      </div>

      {/* Loading State */}
      {isLoading ? (
        <div className="space-y-4 sm:space-y-6">
          {/* Skeleton: tarjeta de temperatura actual */}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 mb-4 sm:mb-6 transition-colors duration-200">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <div>
            <h2 className="text-base sm:text-lg font-semibold text-gray-700 dark:text-gray-200 mb-4">Temperatura Actual</h2>
            <div className="flex items-center gap-4">
              <div className="p-3 sm:p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20 flex-shrink-0">
                <ThermometerIcon className="h-8 sm:h-10 w-8 sm:w-10 text-blue-500" />
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-end gap-1">
                  <span className="text-3xl sm:text-4xl font-bold text-gray-800 dark:text-white truncate">
                    {isFiniteNum(latestTemp) ? round1(latestTemp as number) : 'N/D'}
                  </span>
                  <span className="text-lg sm:text-xl text-gray-500 dark:text-gray-400 flex-shrink-0">°C</span>
                </div>
                <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">
                  ...
                  {(() => {
                    ...
                    // Mostrar 'actual' solo si realmente estamos online
                    if (isOnline) {
            ...
            <div className="mt-4">
              <div className="flex justify-between text-xs sm:text-sm mb-1">
                ...
                <span className="font-medium text-gray-700 dark:text-gray-300">20°C - 26°C</span>
              </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 mb-4 sm:mb-6 transition-colors duration-200">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-4">
          <h2 className="text-base sm:text-lg font-semibold text-gray-800 dark:text-white">Gráfico de Temperatura</h2>
        ...
        <div className="h-64 sm:h-80 overflow-x-auto -mx-4 sm:-mx-6 px-4 sm:px-0">
          <ResponsiveContainer width="100%" height="100%" minWidth={300}>
            <LineChart data={chartData} margin={{ top: 10, right: 30, left: 20, bottom: 10 }}>
              ...
              />
              <YAxis tick={{ fontSize: 12, fill: '#64748b' }} axisLine={{ stroke: '#e5e7eb' }} tickLine={false} domain={['dataMin - 1', 'dataMax + 1']} />
              <Tooltip
                labelFormatter={(ts) => {
